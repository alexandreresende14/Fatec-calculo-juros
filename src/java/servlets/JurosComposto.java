...
public class JurosComposto extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            double valor = 0, juros = 0, meses = 0;
            String error = null;
            try{
                String p1 = request.getParameter("valor");
                String p2 = request.getParameter("juros");
                String p3 = request.getParameter("meses");
                valor = Double.parseDouble(p1);
                juros = Double.parseDouble(p2);
                meses = Double.parseDouble(p3);
            } catch(Exception e){
                error = e.getMessage();
            }
            ...
            if(error != null) {
                ...
                out.printf("%s %.2f", "valor total do juros simples e: ", valor * (Math.pow(1 + juros, meses)));
            }
            out.println("</body>");
            out.println("</html>");
        }
